...
impl Png{
    ...
    pub fn append_chunk(&mut self, chunk: Chunk) {
        let last_idx_before_iend = self.chunks.len().saturating_sub(1);
            self.chunks.insert(last_idx_before_iend, chunk);
    }
...
impl TryFrom<&[u8]> for Png{
    ...
    fn try_from(bytes: &[u8]) -> Result<Self> {
        ...
        while i < bytes.len(){
            if i + 8 >= bytes.len(){
                bail!("Unexpected end of data while reading chunk");
            }

            let length = u32::from_be_bytes(bytes[i..i+4].try_into()?);
            i += 4;

            let chunk_total = 4 + length as usize + 4;

            if i + chunk_total > bytes.len(){
                bail!("Chunk declares more data than available!");
            }

            let chunk = Chunk::try_from(&bytes[i - 4..(i + chunk_total)])?;
            chunks.push(chunk);
...
impl Display for Png{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        writeln!(f, "The Header: {{")?;
        writeln!(f, "{:?}", Png::STANDARD_HEADER)?;
        writeln!(f, "}}")?;
        writeln!(f, "Chunks: {{")?;
